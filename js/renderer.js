...
const Renderer = (() => {
  ...
  const LAYOUTS = {
    ...
    portrait: {
      ...
      deckX: 100, deckY: 98, deckW: 40, deckH: 22,
      doorX: 62, doorY: 76,
      ...
      // Stage — Lyra on deck (deck attaches at trailer right edge ~x=124)
      lyraX: 105, lyraY: 58,
      ampX: 100, ampY: 92,
      // Grill
      grillX: 5, grillY: 105,
  ...
  const backgrounds = {
    ...
    _biltroy_structure(ctx, palette) {
      ...
      // Transform for trailer: translate + scale to fit portrait
      const trailerScale = L.trailerW / 120; // landscape trailer is 120px
      ctx.save();
      ctx.translate(L.trailerX, L.trailerY);
      ctx.scale(trailerScale, trailerScale);
      // Draw in landscape-local coords (trailer at 0,0 instead of 70,88)
      ctx.translate(-70, -88);
  ...
  function drawDialogueBox(characterName, nameColor, lineCount) {
    const x = DIALOG_MARGIN;
    const w = INTERNAL_W - DIALOG_MARGIN * 2;
    // Auto-size height: name line + text lines + padding
    const nameH = (PixelFont.CHAR_H + 3) * TEXT_SCALE;
    const lineH = (PixelFont.CHAR_H + 2) * TEXT_SCALE;
    const numLines = Math.max(1, lineCount || 2);
    const h = TEXT_PADDING * 2 + nameH + numLines * lineH + 2;
    const y = INTERNAL_H - h - DIALOG_MARGIN;
  ...
  function drawDialogueText(twState, lineCount) {
    const x = DIALOG_MARGIN + TEXT_PADDING;
    const nameH = (PixelFont.CHAR_H + 3) * TEXT_SCALE;
    const lineHeight = (PixelFont.CHAR_H + 2) * TEXT_SCALE;
    const numLines = Math.max(1, lineCount || 2);
    const h = TEXT_PADDING * 2 + nameH + numLines * lineHeight + 2;
    const boxY = INTERNAL_H - h - DIALOG_MARGIN;
    const startY = boxY + TEXT_PADDING + nameH;
  ...
  function drawCreditsOverlay(state) {
    ...
    // Dark background so text is readable over the scene
